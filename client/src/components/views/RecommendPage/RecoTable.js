...
function RecoTable() {
    ...
    useEffect(() => {
        setLoading(true)
        //Axios.get('/api/esgdata/getEsgdata') //('/api/esgdata/esgdatas') //데이터 가져오기 //수정해!
        Axios.get('/api/recommend/recommends')
            .then(response  => {
                if(response.data.success) {
                    setLoading(false)
                    console.log(response.data)
                    //setDataSource(response.data.esgdata)
                    setDataSource(response.data.recommend) //No idee,,,
                } else {
                    alert('추천 데이터 가져오기를 실패')
                }
            })
    }, [])
    ...
    // const renderCards = dataSource.map((comp, index) => {
